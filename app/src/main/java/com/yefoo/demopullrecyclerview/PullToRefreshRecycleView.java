...
public class PullToRefreshRecycleView extends LinearLayout {
    ...
    private void init(Context context) {
        ...
        //添加HeaderView
        mHeaderView = new RefreshHead(context);
        LayoutParams params = new LayoutParams(-1, -2);
        mHeaderView.setLayoutParams(params);
        addView(mHeaderView, 0);

        //添加FooterView
        mFooterView = new LoadMoreView(context);
...
//                mHeaderView.onNormal();
                mHeaderView.setState(RefreshHead.STATE_NORMAL);
                smoothScrollTo(0);
                break;
...
//                    mHeaderView.onRefreshing();
                    mHeaderView.setState(RefreshHead.STATE_REFRESHING);
                    isRefreshing = true;
                    if (listener != null) {
                        //执行刷新回调
...
//            mHeaderView.onPullToRefresh(Math.abs(scrollY));
            mHeaderView.onMove(Math.abs(scrollY));
...
//        mRefreshView.addItemDecoration(new CustomItemDecoration(mContext, CustomItemDecoration.VERTICAL_LIST));
    ...
    public boolean canScrollVertically(int direction) {
        final int offset = mRefreshView.computeVerticalScrollOffset();
        final int range = mRefreshView.computeVerticalScrollRange() - mRefreshView.computeVerticalScrollExtent();
